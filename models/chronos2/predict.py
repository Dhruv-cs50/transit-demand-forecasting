...
def main():
    ...
    if args.all:
        ...
        # forecast_all_stations() returns an empty pd.DataFrame() (no columns
        # at all) when every station's forecast failed -- indexing
        # combined['station_id'] on that raises KeyError instead of the
        # already-logged "No forecasts generated" being the only signal.
        if combined.empty:
            print("\nBatch complete: 0 rows (no forecasts generated)")
        else:
