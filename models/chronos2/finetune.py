...
# Known-future covariates — shared by _default_covariate_cols() (used to build
# the training frame) and _get_known_covariate_cols() (used to tell AutoGluon
# which columns are known-future) so the two can't drift out of sync.
_KNOWN_FUTURE_COLS = [
...
def _default_covariate_cols(df: pd.DataFrame) -> list[str]:
    ...
    Past only (only available up to the forecast origin):
      ...
      - Rolling statistics
    """
    known_future = _KNOWN_FUTURE_COLS
...
def _get_known_covariate_cols(ts_df: "TimeSeriesDataFrame") -> list[str]:
    ...
    return [c for c in _KNOWN_FUTURE_COLS if c in ts_df.columns]
