...
def add_weather_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    _onset_grp = "station_id" if "station_id" in df.columns else [True] * len(df)
    _prev_raining = df.groupby(_onset_grp)["is_raining"].transform(
        lambda x: x.shift(1).fillna(False)
    )
    df["is_rain_onset"] = df["is_raining"] & ~_prev_raining
