...
def _run_forecast(
    ...
    for _, row in pred_df.iterrows():
        ...
        results.append({
            "timestamp": str(ts),
            "p10": max(0.0, float(row[q_cols["p10"]])) if q_cols["p10"] else 0.0,
            "p50": max(0.0, float(row[q_cols["p50"]])) if q_cols["p50"] else 0.0,
            "p90": max(0.0, float(row[q_cols["p90"]])) if q_cols["p90"] else 0.0,
        })
